...
Game::Game() :
	wildcardTree(false),
	offlineTrainingWindow(std::numeric_limits<uint32_t>::max(), "Choose a option", "Please choose a option:")
{
	...
	offlineTrainingWindow.choices.emplace_back("Sleep", SKILL_SWORD);
	...
	if (player) {
		...
		if (offlineTrainingSkill != -1) {
			...
			if (offlineTime >= 600) {
				...
				if (offlineTrainingTime >= 6000000) {
					std::ostringstream ss;
					ss << "During your absence you sleep for ";
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		if (attackerPlayer && targetPlayer) {
			...
			if (attackerPlayer->getSkull() == SKULL_RED && attackerPlayer->getSkullClient(targetPlayer) == SKULL_NONE) {
				return false;
			}
		}
		...
		if (target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
			...
			if (manaDamage != 0) {
				...
				std::string spectatorMessage = ucfirst(target->getNameDescription()) + " loses " + damageString + " energy";
				if (attacker) {
				...
				for (Creature* spectator : list) {
					...
					if (tmpPlayer->getPosition().z == targetPos.z) {
						if (tmpPlayer == attackerPlayer && attackerPlayer != targetPlayer) {
							...
							message.text = ucfirst(target->getNameDescription()) + " loses " + damageString + " energy blocking your attack.";
						} else if (tmpPlayer == targetPlayer) {
							message.type = MESSAGE_DAMAGE_RECEIVED;
							if (!attacker) {
								message.text = "You lose " + damageString + " energy.";
							} else if (targetPlayer == attackerPlayer) {
								message.text = "You lose " + damageString + " energy blocking an attack by yourself.";
							} else {
								message.text = "You lose " + damageString + " energy blocking an attack by " + attacker->getNameDescription() + '.';
							}
						} else {
...
void Game::playerAnswerModalWindow(uint32_t playerId, uint32_t modalWindowId, uint8_t button, uint8_t choice)
{
	...
	if (modalWindowId == std::numeric_limits<uint32_t>::max()) {
		if (button == 1) {
			...
			player->sendTextMessage(MESSAGE_EVENT_ADVANCE, "Sleep aborted.");
		}
