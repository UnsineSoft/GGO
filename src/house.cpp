...
void House::updateDoorDescription() const
{
	...
	if (owner != 0) {
		...
		if (housePrice != -1) {
			ss << " It costs " << (houseTiles.size() * housePrice) << " dollars.";
		}
	}
	...
	for (const auto& it : houseMap) {
		...
		if (player.getBankBalance() >= rent) {
			...
			if (house->getPayRentWarnings() < 7) {
				...
				std::ostringstream ss;
				ss << "Warning! \nThe " << period << " rent of " << house->getRent() << " dollars for your house \"" << house->getName() << "\" is payable. Have it within " << daysLeft << " days or you will lose this house.";
