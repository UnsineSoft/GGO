                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
		...
		if (it.weaponType == WEAPON_DISTANCE && it.ammoType != AMMO_NONE) {
			...
			if (it.abilities) {
				...
				if (it.abilities->stats[STAT_MAGICPOINTS]) {
					...
					s << "dexterity " << std::showpos << it.abilities->stats[STAT_MAGICPOINTS] << std::noshowpos;
				}
	...
	if (it.wieldInfo != 0) {
		...
		if (it.wieldInfo & WIELDINFO_MAGLV) {
			...
			s << " dexterity " << it.minReqMagicLevel << " or higher";
		}
...
std::string Item::getWeightDescription(const ItemType& it, double weight, uint32_t _count /*= 1*/)
{
	...
	if (it.stackable && _count > 1 && it.showCount != 0) {
		ss << "They weigh " << std::fixed << std::setprecision(2) << weight << " kg.";
	} else {
		ss << "It weighs " << std::fixed << std::setprecision(2) << weight << " kg.";
	}
