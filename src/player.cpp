...
int32_t Player::getWeaponSkill(const Item* item) const
{
	...
	switch (weaponType) {
		...
		case WEAPON_FIST: {
							 attackSkill = getSkill(SKILL_FIST, SKILLVALUE_LEVEL);
							 break;
		}
...
void Player::addSkillAdvance(skills_t skill, uint32_t count)
{
	...
	while ((skills[skill][SKILLVALUE_TRIES] + count) >= nextReqTries) {
		...
		ss << "You have raised your " << getSkillName(skill) << " to " << skills[skill][SKILLVALUE_LEVEL] << '.';
...
void Player::dropLoot(Container* corpse, Creature* _lastHitCreature)
{
	if (corpse && lootDrop && vocation->getId() != VOCATION_NONE) {
		...
		if (inventory[CONST_SLOT_NECKLACE] && inventory[CONST_SLOT_NECKLACE]->getID() == ITEM_AMULETOFLOSS) {
			...
			for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_FEET; ++i) {
				Item* item = inventory[i];
				if (item) {
					if (playerSkull == SKULL_BLACK || uniform_random(1, (item->getContainer() ? 100 : 1000)) <= getDropPercent()) {
	...
	if (value != -1) {
		...
		if (!isLogin) {
			...
			if (lastQuestlogUpdate != currentFrameTime && Quests::getInstance()->isQuestStorage(key, value, oldValue)) {
				...
				sendTextMessage(MESSAGE_EVENT_ADVANCE, "Mission updated, check your questlog for more information.");
			}
		}
	} else {
...
void Player::addManaSpent(uint64_t amount)
{
	...
	while ((manaSpent + amount) >= nextReqMana) {
		...
		std::ostringstream ss;
		ss << "You have raised your dexterity to " << magLevel << '.';
		sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/, bool applyStaminaChange/* = false*/, bool applyMultiplier/* = false*/)
{
	...
	if (prevLevel != level) {
		...
		std::ostringstream ss;
		//ss << "You have advanced from Level " << prevLevel << " to Level " << level << '.';
		//sendTextMessage(MESSAGE_EVENT_ADVANCE, ss.str());
	}
...
void Player::regenerateStamina(int32_t offlineTime)
{
	...
	if (regainStaminaMinutes > maxNormalStaminaRegen) {
		int16_t happyHourStaminaRegen = (offlineTime - (maxNormalStaminaRegen * 90)) / 300;
