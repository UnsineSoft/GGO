...
void ProtocolGame::login(const std::string& name, uint32_t accountId, OperatingSystem_t operatingSystem)
{
	...
	if (!_player || g_config.getBoolean(ConfigManager::ALLOW_CLONES)) {
		...
		if (g_game.getGameState() == GAME_STATE_CLOSED && !player->hasFlag(PlayerFlag_CanAlwaysLogin)) {
			disconnectClient("Server is currently closed / El servidor esta cerrado actualmente.");
			return;
		}
...
#define dispatchDisconnectClient(err) g_dispatcher.addTask(createTask(std::bind(&ProtocolGame::disconnectClient, this, err)))
	...
	if (version < CLIENT_VERSION_MIN || version > CLIENT_VERSION_MAX) {
		dispatchDisconnectClient("Do you need to download the newest GGO client to play!");
		return;
	}
...
void ProtocolGame::sendMarketDetail(uint16_t itemId)
{
	...
	if (it.abilities) {
		...
		if (it.abilities->stats[STAT_MAGICPOINTS] != 0) {
			...
			ss << "dexterity " << std::showpos << it.abilities->stats[STAT_MAGICPOINTS] << std::noshowpos;
		}
...
void ProtocolGame::sendOutfitWindow()
{
	...
	if (player->isAccessPlayer()) {
		static const std::string gamemasterOutfitName = "Administrator";
